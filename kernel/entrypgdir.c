...
/* 
 * The entry.S page directory maps the first 4MB of physical memroy
 * starting at virtual address KERNBASE (that is, it maps virtual
 * addresses [KERNBASE, KERNBASE+4MB) to physical addresses [0, 4MB)),
 * We choose 4MB because that's how much can map with one page table
 * and it'e enough to get us through early boot. We also map virtual
 * addresses [0, 4MB) to physical address [0, 4MB); this region is
 * critical for a few instructions in entry.S and we never use it again.
 ...
 * Entry 1 of the page table maps to physical page 1, etc.
 */
__attribute__((__aligned__(PGSIZE)))
