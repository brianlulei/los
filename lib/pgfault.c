 ...
 * wrapper in pfentry.S, which in turns calls the registered C function.
 */

#include <include/lib.h>

/* Assemply language pgfault entrypoint defined in lib/pfentry.S */
...
set_pgfault_handler(void (*handler)(struct UTrapframe *utf))
{
	...
	if (_pgfault_handler == 0) {
		// First time called
		r = sys_page_alloc(thisenv->env_id, (void *)(UXSTACKTOP-PGSIZE), 
						   PTE_U | PTE_P | PTE_W);
		if (r < 0)
